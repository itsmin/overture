...
Read this project's CLAUDE.md, then work through each applicable step.
...
Domain-specific health checks. Examples from production:

- **Git status**: Branch, clean/dirty state, unpushed commits
- **TypeScript/lint**: Quick typecheck (timeout after 15 seconds)
...
2. Read CURRENT PRIORITIES from CLAUDE.md.
...
[Full prioritized plan with reasoning — present as-is from CLAUDE.md, don't compress]
...
- **Automation option**: Steps 0-2 can be automated with shell hooks. See `templates/hooks/` for the pattern.
